...
function partition_genome_coordinates(gff3_path::String, perigenic_pad::Int64=500)
    ...
    SequenceSampler.build_feature_df!(gff3_path, "supercontig", "MT", scaffold_df)
    SequenceSampler.build_feature_df!(gff3_path, "chromosome", "MT", scaffold_df)
    SequenceSampler.add_metacoordinates!(scaffold_df)
    ...
    SequenceSampler.build_feature_df!(gff3_path, "exon", "MT", exon_df)
    ...
    for scaffold_subframe in groupby(exon_df, :SeqID) # for each scaffold subframe that has exon features
    ...
    SequenceSampler.build_feature_df!(gff3_path, "gene", "MT", gene_df)
    ...
    for scaffold_subframe in groupby(scaffold_df, :SeqID) # for each scaffold subframe that has exon features
        scaffold_id  = scaffold_subframe.SeqID[1] #get the scaffold id
    ...
    for scaffold_subframe in groupby(gene_df, :SeqID) # for each scaffold subframe that has exon features
        scaffold_id  = scaffold_subframe.SeqID[1] #get the scaffold id
        scaffold_bitarray = init_scaffold_bitarray(scaffold_id, scaffold_df, false) #init a bitarray of scaffold length
        project_features_to_bitarray!(scaffold_subframe,scaffold_bitarray) #project features as Trues on bitarray of falses
    ...
    return ("exon", merged_exon_df), ("periexonic",periexonic_df), ("intergenic",intergenic_df)
end
